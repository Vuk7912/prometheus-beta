...
def test_basic_near_palindrome_pairs():
    ...
    result = find_near_palindrome_pairs(["abcde", "edcba", "hello", "olleh"])
    assert len(result) > 0
    assert ["abcde", "edcba"] in result or ["edcba", "abcde"] in result
...
def test_multiple_near_palindrome_pairs():
    ...
    result = find_near_palindrome_pairs(["abcde", "edcba", "abxde", "exdba"])
    assert len(result) > 1
